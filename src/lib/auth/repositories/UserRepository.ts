...
class MongoUserRepository implements UserRepository {
	...
	public async update(filter: UserFilter, inp: UpdateUser): Promise<number> {
		...
		try {
			res = await this.UserCollection.updateMany(
				mongoFilter,
				updateFilter
			);
		} catch (err) {
...
export async function MongoUserRepositoryFactory(db: Db) {
	/** Create collection */
	if (
		typeof (await db.collections()).find(
			(collection) =>
				collection.collectionName === MongoUserRepository.collectionName
		) === "undefined"
	) {
		await db.createCollection(MongoUserRepository.collectionName);
	}
	...
	if (
		typeof indexes.find(
			(indexDescription) =>
				indexDescription.name === "username_uniqueness"
		) === "undefined"
	) {
